...
client.on("message", async message => {
	if (db.get("autogive") == "1") {
	  if (message.author.id != "OTO GÖNDERİM HESAP ID") return; 
...
client.on("message", async message => {	
	if (db.get("ownerid") != message.author.id) return;
		if (message.content.split(" ")[0] == "!yardım") {
			message.channel.send(`:robot: **Bot Komutları**
			> **!başlat** veya **!aktifet** |-| Botunuzu aktif edersiniz.
			> **!durdur** |-| Botunuzu durdurursunuz. (activate your bot)
			...
			> -
`);
	await message.channel.send(`:moneybag: **OwO Bot Komutları**
	...
	await message.channel.send(`:gear: **Ayar Komutları**
...
if (message.content.split(" ")[0] == "!captcha-koruma-bilgi" ) {
	...
	message.channel.send(`:shield: **Captcha Koruma Bilgilendirme** 
...
if(message.content.split(" ")[0] == "!ayarlar") {
	...
	message.channel.send(`:globe_with_meridians: **Genel Ayarlar** 
